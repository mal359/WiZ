...
extern BOOL bAbort;             /* defined in Wizdiff.c  Read only here */
...
complist_filedialog(VIEW view)
{
        ...
        complist_match(cl, view, FALSE, FALSE);//TRUE);
...
complist_args(LPSTR p1, LPSTR p2, VIEW view, BOOL fDeep)
{
        ...
        complist_match(cl, view, fDeep, FALSE);//TRUE);
...
*/
        ...
        if (fh < 0) {
                ...
                Wizdiff_UI(TRUE);
                MessageBox(NULL, msg, "WizDiff", MB_ICONSTOP|MB_OK);
                Wizdiff_UI(FALSE);
                return;
        }
...
*/
        ...
        List_TRAVERSE(cl->items, ci) {
                ...
                if (Poll()) {
                        ...
                        MessageBox(hwndClient, "Copy Aborted",
                                "WizDiff", MB_OK|MB_ICONINFORMATION);
                        Wizdiff_UI(FALSE);
                        break;
                }
        ...
        Wizdiff_UI(TRUE);
        MessageBox(hwndClient, buffer, "WizDiff", MB_OK|MB_ICONINFORMATION);
        Wizdiff_UI(FALSE);
