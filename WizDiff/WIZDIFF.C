...
*
* Overview of WizDiff internals - the whole program.
*
* WizDiff is built from several modules (a "module" has a .h file
...
*
*    WizDiff  - main program - parse arguments, put up main window,
...
char editor_cmdline[256] = "Wizeditor %p";//"notepad %p";  /* editor cmdline */
...
InitApplication(HINSTANCE hInstance)
{
        ...
        wc.hIcon = LoadIcon(hInstance, "WizDiff");
        wc.hCursor = LoadCursor(NULL, IDC_ARROW);
        wc.hbrBackground = NULL;
        wc.lpszClassName =  "WizDiffViewerClass";
...
InitInstance(HINSTANCE hInstance, int nCmdShow)
{
        ...
        hMenu = LoadMenu(hInstance, "WizDiffMenu");
        haccel = LoadAccelerators(hInstance, "WizDiffAccel");

        /* create the main window */
        hwndClient = CreateWindow("WizDiffViewerClass",
                            "WizDiff",
                            WS_OVERLAPPEDWINDOW,
                            CW_USEDEFAULT,
                            CW_USEDEFAULT,
 ...
 */
void
Wizdiff_usage(LPSTR msg)
{
        int retval;
        if (msg==NULL)
                msg = "WizDiff {-L} path1 {path2} {-s{slrd} savefile}";

        retval = MessageBox(hwndClient,
                msg,
                "WizDiff Usage", MB_ICONINFORMATION|MB_OKCANCEL);
        if (retval == IDCANCEL) {
                exit(1);
        }
}
...
CreateTools(void)
{
        ...
        InitializeCriticalSection(&CSWizDiff);
...
DeleteTools(void)
{
        ...
        DeleteCriticalSection(&CSWizDiff);
...
DoPrint(void)
{
        ...
        /* header is filenames or just WizDiff if no names known*/
...
FindNextChange(void)
{
        ...
        if (row >= 0) {
                ...
                Wizdiff_UI(TRUE);
                MessageBox(hwndClient, "No More Changes", "WizDiff",
...
FindPrevChange(void)
{
        ...
        if (row >= 0) {
                ...
                Wizdiff_UI(TRUE);
                MessageBox(hwndClient, "No Previous Changes", "WizDiff",
                        MB_ICONINFORMATION|MB_OK);
                Wizdiff_UI(FALSE);
...
SetBusy(void)
{
         ...
         */
        SetWindowText(hwndClient, "WizDiff: scanning");
...
SetNotBusy(void)
{
        ...
        SetWindowText(hwndClient, "WizDiff");
...
MainWndProc(HWND hWnd, UINT message, UINT wParam, LONG lParam)
{
        ...
        if ((message == RegisteredMessage) && fLaunched)
           message = WM_CLOSE;
