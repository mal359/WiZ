...
for (icmd = 0; icmd < ccmd; icmd++)
    ...
    if (rgcmd[icmd].wCommand == wCommand)
       ...
       return rgcmd[icmd].pfncmd(hwnd,
                                 wCommand,
                                 GET_WM_COMMAND_CMD(wparam, lparam), // HIWORD(wparam)
//                                 wparam,
                                 GET_WM_COMMAND_HWND(wparam, lparam)); // (HWND)lparam
//                                 lparam);
       }
    }
...
//
//  PARAMETERS:
//    edwp - Enumerate specifying the appropriate default window procedure.
//    hwnd - The window handle
//    uMessage - The message number
//    wparam - Message specific data
//    lparam - Message specific data
...
switch (edwp)
    ...
    case edwpMDIFrame:
         return DefFrameProc(hwnd, hWndListParent, uMessage, wparam, lparam);
    case edwpMDIChild:
